<template>
  <div class="relative w-[1080px] h-[1080px] overflow-hidden bg-[#050816] text-white font-['Inter',sans-serif]">
    ...
    <div class="relative z-10 flex h-full flex-col px-12 py-10">
      <header class="flex items-center justify-between gap-4">
        <div class="flex min-w-0 items-center gap-4">
          ...
          <p class="text-2xl font-black leading-none tracking-[-0.06em] text-white sm:text-3xl">{{ teamName }}</p>
        ...
        <div class="flex items-center gap-5 text-white">
          <div class="space-y-1 text-right">
            ...
            <p class="text-[2.35rem] font-black leading-none tracking-[-0.07em] text-white">GW {{ gameweekLabel }}</p>
          </div>
          <div class="space-y-1 text-right">
            <p class="text-[0.95rem] font-semibold uppercase tracking-[0.18em] text-white/38">Price</p>
            <p class="text-[2.35rem] font-black leading-none tracking-[-0.07em] text-white">{{ formatCost(player.now_cost) }}</p>
          </div>
      ...
      <main class="mt-8 flex flex-1 gap-6 xl:gap-8">
        ...
        <section class="flex min-w-0 flex-1 basis-0 flex-col gap-5">
          <div class="flex shrink-0 items-center gap-3 whitespace-nowrap text-[1rem] font-semibold uppercase tracking-[0.16em] text-white/55">
          ...
          <div class="grid grid-cols-2 gap-x-6 gap-y-2 pt-1 text-[1rem] leading-7 text-white/78 pt-6">
            <div v-for="item in signalStats" :key="item.label"
              ...
              <span class="text-[0.92rem] font-semibold uppercase tracking-[0.14em] text-white/34">{{ item.label }}</span>
              <span class="font-bold tracking-[-0.02em]" :class="item.className || 'text-white'">{{ item.value }}</span>
            </div>
          </div>

          <div class="mt-auto pb-4">
            <div class="mb-6 pt-1">
              <div class="flex items-center justify-between gap-3 text-[1rem] font-semibold uppercase tracking-[0.14em] text-white/42">
              ...
              <div class="mt-2 flex items-center gap-2">
                <div v-for="item in cardMarks" :key="item.label" class="flex h-12 min-w-0 items-center gap-2 px-3"
                  ...
                  <div class="min-w-0">
                    <div class="text-[0.92rem] font-semibold uppercase tracking-[0.14em] opacity-70">{{ item.label }}</div>
            ...
            <div class="flex items-end justify-between gap-4">
              <div>
                <p class="text-[0.95rem] font-semibold uppercase tracking-[0.16em] text-lime-100/55">Transfers in</p>
                <p class="mt-1 text-4xl font-black tracking-[-0.07em] text-lime-200">{{ formatCompact(transfersIn) }}</p>
              </div>
              <div class="text-center">
                <p class="text-[0.95rem] font-semibold uppercase tracking-[0.16em] text-white/40">Net</p>
                <p class="mt-1 text-4xl font-black tracking-[-0.07em]" :class="netValueClass">{{ formatSigned(netMovement) }}</p>
              </div>
              <div class="text-right">
                <p class="text-[0.95rem] font-semibold uppercase tracking-[0.16em] text-sky-100/55">Transfers out</p>
                <p class="mt-1 text-4xl font-black tracking-[-0.07em] text-sky-200">{{ formatCompact(transfersOut) }}</p>
              </div>
            </div>

            <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10 flex">
              <div class="h-full bg-lime-300 transition-all duration-500" :style="{ width: `${transferInShare}%` }" />
              <div class="h-full bg-sky-300 transition-all duration-500" :style="{ width: `${transferOutShare}%` }" />
            </div>

            <div class="mt-2 flex items-center justify-between text-[0.9rem] font-semibold uppercase tracking-[0.16em] text-white/40">
              <span>FantasyPL</span>
        ...
        <section class="relative flex min-h-0 min-w-0 flex-1 basis-0 flex-col overflow-hidden rounded-[40px] border border-white/10 bg-[#050816]/[0.03] shadow-[0_24px_70px_rgba(0,0,0,0.2)]">
          <div class="absolute inset-0 bg-[radial-gradient(circle_at_50%_18%,rgba(190,255,84,0.18),transparent_26%),radial-gradient(circle_at_80%_12%,rgba(63,195,255,0.16),transparent_24%),linear-gradient(180deg,rgba(255,255,255,0.02)_0%,rgba(255,255,255,0.01)_48%,rgba(0,0,0,0.28)_100%)]" />
          <div class="absolute inset-x-0 top-0 h-28 bg-gradient-to-b from-[#050816]/50 to-transparent" />
          <div class="absolute inset-x-0 bottom-0 h-[42%] bg-gradient-to-t from-[#050816] via-black/85 to-transparent" />
          ...
          <div class="relative z-10 flex h-full flex-col p-7">
            <div class="space-y-2">
              ...
              <p class="text-[1rem] font-medium uppercase tracking-[0.16em] text-white/55">GW {{ gameweekLabel }} / {{ teamName }} / {{ positionLabel }}</p>
            </div>

            <div class="relative mt-5 flex min-h-0 flex-1 items-end justify-center overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05)_0%,rgba(8,12,24,0.12)_40%,rgba(0,0,0,0.95)_100%)]">
              ...
              <div class="absolute inset-x-[18%] bottom-0 h-2/5 bg-gradient-to-t from-black via-black/80 to-transparent" />
            ...
            <div class="mt-12 grid grid-cols-2 gap-3 text-white">
              ...
              <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur">
                ...
                <p class="mt-2 text-2xl font-black tracking-[-0.05em] text-white">{{ formatPercent(player.selected_by_percent) }}</p>
              </div>
              <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur">
                <p class="text-[0.9rem] font-semibold uppercase tracking-[0.16em] text-white/38">Minutes</p>
                <p class="mt-2 text-2xl font-black tracking-[-0.05em] text-white">{{ formatCompact(player.minutes) }}</p>
              </div>
              <div class="rounded-3xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur">
                <p class="text-[0.9rem] font-semibold uppercase tracking-[0.16em] text-white/38">Points</p>
                <p class="mt-2 text-2xl font-black tracking-[-0.05em] text-white">{{ formatCompact(player.total_points) }}</p>
              </div>
            </div>
          </div>
        </section>
      ...
      <footer class="mt-6 flex items-center justify-between border-t border-white/10 pt-4 text-[0.9rem] font-semibold uppercase tracking-[0.16em] text-white/40">
        <span>FantasyPL.app</span>
        <span>Generated with AI Precision</span>
      </footer>
    </div>
  </div>
</template>
...
const statusLabels = { a: 'Available', d: 'Doubtful', i: 'Injured', s: 'Suspended', u: 'Unavailable' };
const statusToneMap = {
