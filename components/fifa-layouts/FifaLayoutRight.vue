<template>
  <div class="relative w-[1080px] h-[1080px] overflow-hidden bg-[#050816] text-white font-['Outfit',sans-serif]">
    ...
    <div v-if="backgroundImage" class="absolute inset-0">
      ...
      />
      <!-- Dynamic Right-to-left Gradient -->
      <div 
        class="absolute top-0 bottom-0 right-0 bg-gradient-to-l from-black via-black/80 to-transparent" 
        :style="{ 
    ...
    <!-- Main Content Layout -->
    <div class="relative z-10 flex h-full flex-col justify-center items-end text-right p-24">
      
      <div class="space-y-10 max-w-[850px]">
        ...
        <div v-if="buttons && buttons.length" class="flex flex-wrap justify-end gap-6 pt-6">
          <div v-for="(btn, idx) in buttons" :key="idx" class="flex items-center gap-6">
            <div v-if="idx > 0" class="text-4xl font-black text-white/30">+</div>
